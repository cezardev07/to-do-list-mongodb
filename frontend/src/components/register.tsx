...
export const Register = () => {
  ...
  const handleSubmit = async (event: { preventDefault: () => void}) => {
      ...
      try {
        const res = await fetch(import.meta.env.VITE_ENDPOINT + "auth/create",{
          method: "POST",
          headers: {
  ...
  return(
    <div className="max-w-xl min-h-screen mx-auto flex justify-center gap-2 p-2 flex-col">
      <form
        ...
        <button type="submit" className="uppercase rounded-sm bg-blue-700 p-5 hover:bg-blue-600">
          cadastrar
        </button>
        <Link to="/"
          className="text-center hover:bg-green-700 p-5 bg-zinc-900"
        >clique aqui para fazer login</Link>
      </form>
    </div>
  )
}